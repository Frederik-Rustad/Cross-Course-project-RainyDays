...
document.querySelector('.js-products-grid').innerHTML = '<p>Loading...</p>';
...
fetch(productListURL)
  ...
  .then(data => {
    ...
    data.forEach(product => {
      products1HTML += `
        <div class='cattext'>
        <a class='JacketsImg js-product-detail' data-product-id="${product.id}" href='productDetailPage.html'><img src="${product.images[0].src}" alt='Product' class='itemlist js-product-detail' data-product-id="${product.id}"></a>
        <h2>${product.name}</h2>
